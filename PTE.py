    ...
    for ch in ch_list:
        ...
        if ch_ss not in ch:
            ...
            p.line(
                ...
                y,
                line_width=0.5,
                color=clo.channel_colors.get(
                    ch_num,
                    'blue'))
            # print(case.plot_labels.get(ch,[]))
            # print(ch)
            source = pd.DataFrame.from_records(
                case.plot_labels.get(
                    ch,
                    []),
                columns=[
                    'x',
            ...
            plot_dict = clo.plot_size_standard(
                case, ch, plot_dict, w, h=400, ch_ss_num=5)
...
def find_peaks_in_range(case, ch, x_col_name, start, end, h=100, d=10):
    ...
    peaks = [i for i in peaks if case.df[x_col_name]
             [i] >= start and case.df[x_col_name][i] <= end]
...
def build_allelic_ladder(case, allelic_case_names, ch_ss_num=5):
    if 'Allelic' in case.name:
        ...
        ch_ss = 'channel_' + str(ch_ss_num)
        channels = [
            ch for ch in case.df.columns if 'x_fitted' not in ch and ch_ss not in ch]
        ...
        for ch in channels:
            ...
            for region in alleles[ch_num].keys():
                ...
                choices = find_peaks_in_range(
                    case, ch, x_col_name, start, end, h=120, d=5)
                if len(choices) < len(stock_ladder):
                    print(
                        '\tWARNING: len(choices) = {}, k = {}'.format(
                            len(choices), len(stock_ladder)))
                    # print('\tlen(stock_ladder = {})'.format(len(stock_ladder)))
                    # print('\tlen(choices) = {}'.format(len(choices)))
                else:
                    X = np.array([sorted(list(c))
                                  for c in combinations(choices, len(stock_ladder))])

                    pfit_zx = np.polyfit(stock_ladder, X.T, deg=1, full=True)
                    residuals_zx = pfit_zx[1]
                    X_mean = np.expand_dims(np.mean(X, axis=1), axis=1)
                    R_sq_zx = 1.0 - (np.square(residuals_zx) /
                                     np.sum(np.square(X - X_mean)))
                    # it's zero when a line is being fit to only two points.
                    if len(R_sq_zx) > 0:
                        ranked_R_sq, indices = np.unique(
                            R_sq_zx, return_index=True)
                        indices = indices.tolist()
                        indices.reverse()
                        ladder_i = X[indices[0]]
                        ladder = case.df[x_col_name][ladder_i].to_list()
                        alleles[ch_num][region][case.name] = {
                            'ladder_i': ladder_i, 'ladder': ladder}
                    else:
                        ladder_i = X[0]
                        ladder = case.df[x_col_name][ladder_i].to_list()
                ...
                plot_labels.extend([(x, y, label, None, None) for x, y, label in zip(
                    x_peaks, y_peaks, allelic_labels)])
...
def label_alleles(case, allelic_case, w=900, h=400, ch_ss_num=5):
    ...
    for ch in ch_list:
        ...
        ch_num = re.findall(r'channel_\d$', ch)[0]
        ch_allelic = [
            ch_a for ch_a in allelic_case.df.columns if ch_num in ch_a][0]
        peaks_i, props = find_peaks(
            case.df[ch], height=100, prominence=100, distance=10)
		...
		"""
        plot_label = [
            (x, y, l, lb, rb) for x, y, l, lb, rb in zip(
                X, Y, labels, left_bases, right_bases)]
        plot_label_temp = {}
        ...
        plot_label = [
            (x,
             y,
             plot_label_temp[x],
             case.df[x_col_name][lb],
             case.df[x_col_name][rb]) for x,
            y,
            lb,
            rb in zip(
                X,
...
def main():
    ...
    for case in cases.values():
        ...
        case = clo.baseline_correction_advanced(
            case, ch_ss_num=5, prominence=10)
        # case = clo.baseline_correction_simple(case, ch_ss_num=5)
        ...
        case, allelic_case_names = build_allelic_ladder(
            case, allelic_case_names, ch_ss_num=5)

    allelic_case = cases[allelic_case_names[0]]
    non_allelic_case_names = [
        case_name for case_name,
        case in cases.items() if 'Allelic' not in case_name]
