...
def build_results_dict():
	peaks = {}
	filename = easygui.fileopenbox(msg='Select results file (should end in .csv)')
	if filename == None:
		exit()
	df = use_csv_module(filename)
	df = df[['Sample File Name', 'Marker', 'Allele', 'Area']]
	df.dropna(axis=0, how='any',inplace=True)	# get rid of peaks that aren't assigned an allele
...
def build_profile(res):
	...
	os.chdir(r'X:\Hospital\Genetics Lab\DNA_Lab\3-Oncology Tests\Engraftment\Allele Charts')
	...
	for choice in choices:
		...
		os.chdir(twd)
		output_file_name = easygui.filesavebox(msg='Save As', default=output_file_name)
...
def get_header(template):
	if template.endswith('.xlsx'):
		...
		template = template + 'x'
		wb.SaveAs(template, FileFormat = 51)    #FileFormat = 51 is for .xlsx extension
...
def fix_formatting(filename, header, case_name):
	...
	cells = [ws[c+str(i)] for i in range(1,3) for c in string.ascii_uppercase[0:ws.max_column]]
	for cell in cells:
		cell.font = Font(bold=True)
		cell.alignment = Alignment(horizontal='left')
	# get locations of 'Allele'
